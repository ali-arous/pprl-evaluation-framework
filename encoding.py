...
# encoding.py - Module that contains concrete classes for the encoding methods
#
# Ali Arous
#
...
import numpy  # For the dynamic bit position selection in Record level BFs
import hashing
import bitarray
from framework import EncodingMethod
...
class CLKEncoding(EncodingMethod):
  def __init__(self):
    super().__init__('CLK')
    self.encoder = CryptoLongtermKeyBFEncoding(([1], 2, True, hashing.RandomHashingMethod()))

  def set_encoding_params(self, attr_encode_tuple_list):
    super().set_encoding_params(attr_encode_tuple_list)
    self.encoder = CryptoLongtermKeyBFEncoding(self.attr_encode_tuple_list)

  def encode(self, attr_val_list):
    self.encoder.encode(attr_val_list)


class RBFEncoding(EncodingMethod):
  def __init__(self):
    super().__init__('RBF')
    self.encoder = RecordBFEncoding(([1], 2, True, hashing.RandomHashingMethod(), 1024)) # last param is num_bf_bit

  def set_encoding_params(self, attr_encode_tuple_list):
    super().set_encoding_params(attr_encode_tuple_list)
    self.encoder = RecordBFEncoding(self.attr_encode_tuple_list)
...
class CryptoLongtermKeyBFEncoding():
  ...
  def encode(self, attr_val_list, salt_str_list=None, mc_harden_class=None):
    ...
    for (j, attr_encode_tuple) in enumerate(self.attr_encode_tuple_list):
      ...
      q_gram_set = set([attr_val[i:i+q]+str(attr_num) for i in \
                       range(attr_val_len - qm1)])
      q_gram_set_mc = set([attr_val[i:i+q] for i in \
                          range(attr_val_len - qm1)])

      if (mc_harden_class != None):
        extra_q_gram_set = \
           mc_harden_class.get_other_q_grams_from_lang_model(q_gram_set_mc)
        q_gram_set = q_gram_set | extra_q_gram_set
  ...
  def encode(self, attr_val_list, salt_str_list=None, mc_harden_class=None):
    ...
    for (j, attr_encode_tuple) in enumerate(self.attr_encode_tuple_list):
      ...
      if(abf_len >= num_bf_bit):
        ...
        use_bit_pos_list = random.sample(list(range(abf_len)), num_bf_bit)

      else:  # Sampling with replacement
        numpy.random.seed(attr_num)
        use_bit_pos_list = list(range(abf_len))  # Make sure all bits are included
        more_sample_bits_needed = num_bf_bit - len(use_bit_pos_list)
        use_bit_pos_list += list(numpy.random.choice(list(range(abf_len)),
...
if (__name__ == '__main__'):
  ...
  print('  Testing attribute level Bloom filter encoding...', end=' ')  # - - - - - - -
  ...
  print('  Testing cryptographic long-term key encoding', end=' ')  # - - - - - - - - -
  ...
  for attr_val_list in rec_list:
    ...
    for (q_gram, pos_set) in clk_q_gram_pos_dict.items():
      assert q_gram[:-1] in attr_val_list[0]+attr_val_list[1]
  ...
  print('  Testing record-level Bloom filter encoding...', end=' ')  # - - - - - - - - -
  ...
  print('  Testing CLK-RBF encoding...', end=' ')  # - - - - - - - - - - - - - - - - - -
