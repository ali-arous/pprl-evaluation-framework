...
# hashing.py - Module that contains concrete classes for hashing methods
#
# Ali Arous
#
...
from framework import HashingMethod

# =============================================================================
class RandomHashingMethod(HashingMethod):
  def __init__(self):
    self.hasher = RandomHashing(hashlib.sha1, 1024, 10)

  def set_params(self, bf_len, num_hash_funct):
    super().set_params(bf_len, num_hash_funct)
    self.hasher = RandomHashing(hashlib.sha1, self.bf_len, self.num_hash_funct)

  def hash_q_gram_set(self, q_gram_set):
    return self.hasher.hash_q_gram_set(q_gram_set)
...
class DoubleHashing():
  ...
  def hash_q_gram_set(self, q_gram_set, salt_str=None):
    ...
    for q_gram in q_gram_set:
      ...
      hex_str1 = self.hash_funct1(str(q_gram).encode('utf-8')).hexdigest()
      int1 =     int(hex_str1, 16)

      hex_str2 = self.hash_funct2(str(q_gram).encode('utf-8')).hexdigest()
...
class TripleHashing():
  ...
  def hash_q_gram_set(self, q_gram_set, salt_str=None):
    ...
    for q_gram in q_gram_set:
      ...
      hex_str3 = self.hash_funct3(str(q_gram).encode('utf-8')).hexdigest()
...
class RandomHashing():
  ...
  def hash_q_gram_set(self, q_gram_set, salt_str=None):
    ...
    for q_gram in q_gram_set:
      ...
      hex_str = self.hash_funct(str(q_gram).encode('utf-8')).hexdigest()
...
if (__name__ == '__main__'):
  ...
  print('  Testing double hashing...', end=' ')  # - - - - - - - - - - - - - - - - - - -
  ...
  print('  Testing enchanced double hashing...', end=' ')  # - - - - - - - - - - - - - -
  ...
  print('  Testing triple hashing...', end=' ')  # - - - - - - - - - - - - - - - - - - -
  ...
  print('  Testing random hashing...', end=' ')  # - - - - - - - - - - - - - - - - - - -
